$(document).ready(function(){
    ...
    var imagesClass = ".img-Anchor";
    
    // found this and it worked. had math.floor & random so i used it. not sure about ceil/floor are doing.
    ...
    function reset() {
        $(randomNumberId).text(rand(19, 50));
        $(totalScoreId).text(0);
    ...
    $(imagesClass).on("click", function() {
        ...
        if(parseInt($(totalScoreId).text()) > parseInt($(randomNumberId).text())) {
            ...
            console.log("won");
            $(resultMsgId).text("Congratulations You won!!!");
            reset();
        }
    });
    reset();
